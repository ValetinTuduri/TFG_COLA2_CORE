...
namespace ros
{
SetpointsSelector::SetpointsSelector(rclcpp::Node::SharedPtr node): node_(node), priority_list_({ { "radio_open_loop", 0.0 }, { "open_loop", 0.0 }, { "safety", 0.0 } })
{
  pub_ts_ = node_->create_publisher<cola2_msgs::msg::Setpoints>(cola2::ros::getNamespace(node_) + std::string("/setpoints_selector/thruster_setpoints"), 10);
}

bool SetpointsSelector::acceptSetpoints(const cola2_msgs::msg::Setpoints& msg)
{
  ...
  {
    if (msg.header.frame_id.find(std::get<0>(elem)) != std::string::npos)
    {
