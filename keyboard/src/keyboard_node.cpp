...
public:
  Keyboard():Node("keyboard_node")
  {
    pub_ = this->create_publisher<sensor_msgs::msg::Joy>("joy", 1);
  ...
  void readKeyboardHits()
  {
    ...
    while (!keyboard_requested_shutdown)
    {
      ...
      else
      {
        ...
        RCLCPP_INFO(this->get_logger(), "Key code is %d. Ignoring key", key);
        rclcpp::sleep_for(std::chrono::milliseconds(10));  // Avoid filling the hard drive if getchar returns immediately on error
        continue;
      }
