...
// messages subscribed
#include <cola2_msgs/msg/body_force_req.hpp>                     // force velocity model
#include <cola2_msgs/msg/dvl.hpp>                                // dvl
#include <cola2_msgs/msg/float32_stamped.hpp>                    // force velocity model
#include <geometry_msgs/msg/pose_with_covariance_stamped.hpp>    // usbl
#include <sensor_msgs/msg/fluid_pressure.hpp>                    // depth
#include <sensor_msgs/msg/imu.hpp>                               // imu
#include <sensor_msgs/msg/nav_sat_fix.hpp>                       // gps
#include <sensor_msgs/msg/range.hpp>                             // altitude
#include <std_msgs/msg/float32.hpp>                              // sound velocity
// messages published
#include <cola2_msgs/msg/nav_sts.hpp>                            // custom navigation
#include <diagnostic_msgs/msg/diagnostic_status.hpp>             // diagnostics
#include <geometry_msgs/msg/pose_stamped.hpp>                        // gps_ned, usbl_ned
#include <nav_msgs/msg/odometry.hpp>                                 // odometrys
// services
...
protected:
  ...
  std::shared_ptr<cola2::ros::TransformHandler> tf_handler_;                       //!< provide tfs
  std::shared_ptr<tf2_ros::TransformBroadcaster> tf_broadcast_;   //!< to publish world->vehicle
  ...
  // ROS Publishers
  rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr pub_odom_;             //!< odometry message
  rclcpp::Publisher<cola2_msgs::msg::NavSts>::SharedPtr pub_nav_;              //!< custom navigation message
  rclcpp::Publisher<geometry_msgs::msg::PoseStamped>::SharedPtr pub_gps_ned_;  //!< GPS projected in current NED
  rclcpp::Publisher<geometry_msgs::msg::PoseStamped>::SharedPtr pub_usbl_ned_; //!< USBL projected in current NED

  double last_publication_time_ = 0.0;  //!< Last time navigation was published

  // ROS Services

  rclcpp::Service<std_srvs::srv::Trigger>::SharedPtr srv_reload_ned_;              //!< reload ned config and reuse current filter
  rclcpp::Service<std_srvs::srv::Trigger>::SharedPtr srv_reload_params_;           //!< same as reset navigation
  rclcpp::Service<std_srvs::srv::Trigger>::SharedPtr srv_reset_navigation_;        //!< reloads params and sets the filter to initial state
  rclcpp::Service<std_srvs::srv::Trigger>::SharedPtr srv_set_depth_sensor_offset_; //!< compute the depth sensor offset
  rclcpp::Client<std_srvs::srv::Trigger>::SharedPtr srv_publish_params_;           //!< service to tell param_logger that parameters have been updated
  ...
  // Other
  rclcpp::TimerBase::SharedPtr timer_; //!< loop to keep checking diagnostics
...
protected:
  ...
  // Diagnostics
  std::shared_ptr<cola2::ros::DiagnosticHelper> diagnostic_;   //!< ease publishing diagnostics
...
public:
   ...
   */
  std_srvs::srv::Trigger_Response getConfig(const bool show = false);
...
public:
  ...
  void init_base(const std::shared_ptr<rclcpp::Node>& node);
   ...
   */
  void updatePositionGPSMsg(const sensor_msgs::msg::NavSatFix& msg);
  bool updatePositionGPSMsgImpl(const sensor_msgs::msg::NavSatFix& msg);
   ...
   * \param msg Recevied message
   */
  void updatePositionUSBLMsg(const geometry_msgs::msg::PoseWithCovarianceStamped& msg);
  bool updatePositionUSBLMsgImpl(const geometry_msgs::msg::PoseWithCovarianceStamped& msg);
  /**
   * \brief Callback to update the filter with a depth message
   *
   * \param msg Recevied message
   */
  void updatePositionDepthMsg(const sensor_msgs::msg::FluidPressure& msg);
  /**
   * \brief Callback to update the filter with a DVL message
   *
   * \param msg Recevied message
   */
  void updateVelocityDVLMsg(const stonefish_ros2::msg::DVL& msg);
  /**
   ...
   * \param msg Recevied message
   */
  void updateVelocityDVLFallbackMsg(const stonefish_ros2::msg::DVL& msg);
  /**
   ...
   */
  void updateVelocityDVLMsgImpl(const stonefish_ros2::msg::DVL& msg, const bool from_dvl);
  /**
   * \brief Callback to update the filter with an IMU message
   *
   * \param msg Recevied message
   */
  void updateIMUMsg(const sensor_msgs::msg::Imu& msg);
  // Others
  /**
   ...
   * \param msg Recevied message
   */
  void updateSoundVelocityMsg(const cola2_msgs::msg::Float32Stamped& msg);
  /**
   * \brief Callback to update the filter with an altitude message
   *
   * \param msg Recevied message
   */
  void updateAltitudeMsg(const sensor_msgs::msg::Range& msg);
   ...
   */
  void publishNavigation(const rclcpp::Time stamp);
  /**
   ...
   */
  void publishGPSNED( const rclcpp::Time stamp, const Eigen::Vector3d& ned, const bool& valid) const;
  /**
   ...
   */
  void publishUSBLNED(const rclcpp::Time stamp, const Eigen::Vector3d& ned, const USBLStatus& status) const;
   ...
   */
  bool srvReloadNED(const std::shared_ptr<rmw_request_id_t> request_header, const std::shared_ptr<std_srvs::srv::Trigger::Request> req, std::shared_ptr<std_srvs::srv::Trigger::Response> res);
  /**
   ...
   * \return Correct processing of the service
   */
  bool srvResetNavigation(const std::shared_ptr<rmw_request_id_t> request_header, const std::shared_ptr<std_srvs::srv::Trigger::Request> req, std::shared_ptr<std_srvs::srv::Trigger::Response> res);
  /**
   ...
   * \return Correct processing of the service
   */
  bool srvSetDepthSensorOffset(const std::shared_ptr<rmw_request_id_t> request_header, const std::shared_ptr<std_srvs::srv::Trigger::Request> req, std::shared_ptr<std_srvs::srv::Trigger::Response> res);
