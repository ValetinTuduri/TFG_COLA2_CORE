 ...
 *
 * In this case the state contains position [x y z] and velocities [u v w]. Orientation and anglular velocity are saved
 ...
 */
class EKFPosition : public EKFBaseROS
{
private:
  // Auxiliary states not in the state vector x
  Eigen::Vector3d vel_ = Eigen::Vector3d::Zero();                 //!< Current filter velocity
  Eigen::Matrix3d cov_vel_ = 0.02 * Eigen::Matrix3d::Identity();  //!< Current filter velocity
...
public:
   ...
   */
  explicit EKFPosition(const bool online = true);
  /**
   * \brief Destructor.
   */
  ~EKFPosition() = default;

  void init_p(const std::shared_ptr<rclcpp::Node>& node);
