...
void EKFPosition::init_p(const std::shared_ptr<rclcpp::Node>& node)
{
  init_base(node);
}
...
bool EKFPosition::updatePositionXY(const double, const Eigen::Vector2d& pose_xy, const Eigen::Matrix2d& cov)
{
  ...
  if (!init_ekf_)
  {
    ...
    RCLCPP_INFO_ONCE(node_->get_logger(),"gps init");
    return true;
  }
  // Update
...
bool EKFPosition::updatePositionZ(const double, const Eigen::Vector1d& pose_z, const Eigen::Matrix1d& cov)
{
  ...
  if (!init_ekf_)
  {
    x_(2) = pose_z(0);
    init_depth_ = true;
    RCLCPP_INFO_ONCE(node_->get_logger(),"depth init");
    return true;
  }
  // Update
  unsigned int size = x_.rows();
...
bool EKFPosition::updateOrientation(const double t, const Eigen::Vector3d& rpy, const Eigen::Matrix3d& cov)
{
  ...
  if (!init_ekf_)
  {
    if ((!config_.use_gps_data_ || init_gps_) && (init_depth_) && (init_dvl_))
    {
      ...
      init_ekf_ = true;
      RCLCPP_INFO_ONCE(node_->get_logger(),"ekf init");
    }
    rpy_ = rpy;
    rpy_cov_ = cov;
    init_imu_ = true;
    RCLCPP_INFO_ONCE(node_->get_logger(),"imu init");
    return true;
  }
                                 ...
                                 const bool from_dvl)
{
  ...
  if (!init_ekf_)
  {
    ...
    if (from_dvl)
    {
      ...
      last_vel = vel;
      init_dvl_ = true;
      RCLCPP_INFO_ONCE(node_->get_logger(),"dvl init");
    }
    return true;
  }
  // Update fallback
  ...
  RCLCPP_WARN(node_->get_logger(),"Too big acceleration (%.2f, %.2f, %.2f) in time %.3f with new velocity (%.2f, %.2f, %.2f)", acceleration(0),
